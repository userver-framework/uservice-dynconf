...
namespace uservice_dynconf::handlers::variables::get {
    ...
    std::string Handler::HandleRequestThrow(
        ...
        std::int32_t count = 0;
        for (auto row = result.AsSetOf<uservice_dynconf::models::VariableWithDate>(userver::storages::postgres::kRowTag).begin() + std::min(kOffset, (int32_t)result.Size());
             row < result.AsSetOf<uservice_dynconf::models::VariableWithDate>(userver::storages::postgres::kRowTag).end(); ++row) {
            if(count >= kLimit)
                break;
