...
namespace uservice_dynconf::handlers::variables::post {
    ...
    struct RequestData{
        ...
        bool isValid() const{
          return  !config_value.empty() && !config_name.empty() && !service_name.empty();
        }
    };
            ...
            userver::formats::json::Value  HandleRequestJsonThrow(
                ...
                if (!request_data.isValid()){
                    ...
                    return uservice_dynconf::utils::MakeError(
                            "400", "Fields 'name', 'value' are required");
                }
                try{
                    auto val = userver::formats::json::FromString(json["value"].As<std::string>({}));
                }catch(const userver::formats::json::ParseException& e){
                    http_response.SetStatus(userver::server::http::HttpStatus::kBadRequest);
                    return uservice_dynconf::utils::MakeError(
                            "400", e.what());
                }
                auto result = cluster_->Execute(userver::storages::postgres::ClusterHostType::kMaster,
