...
ServiceCacheContainer::ServicePtr
ServiceCacheContainer::FindServiceByName(std::string service_name) const {
  if (auto c_ptr = userver::utils::FindOrDefault(service_by_name_, service_name,
                                                 nullptr);
      c_ptr) {
    return c_ptr;
  }
...
ServiceCacheContainer::ServicePtr
ServiceCacheContainer::FindService(const Key key) const {
  if (auto c_ptr =
          userver::utils::FindOrDefault(service_by_uuid_, key, nullptr);
      c_ptr) {
    return c_ptr;
  }
  return userver::utils::FindOrDefault(service_by_uuid_,
                                       Key{kDefaultServiceUuid}, nullptr);
}
